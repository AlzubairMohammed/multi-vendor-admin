<template>
  <app-layout>
    <app-navbar :title="'الأخبار'"></app-navbar>
    <div class="w-full py-8 px-4">
      <page-header title="الاخبار" />
      <div class="flex justify-between items-center py-4">
        <div class="relative">
          <input
            type="email"
            class="peer py-3 pl-6 pr-10 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-gray-700 dark:border-transparent dark:text-gray-400 dark:focus:ring-gray-600"
            placeholder="بحث"
            v-model="search_term"
            @input="search()"
          />
...
<script>
  ...
  export default {
    components: { AppNavbar, FwbModal, AppLayout },
    data() {
      return {
        data: {},
        news: [],
    ...
    methods: {
      ...
      async add() {
        ...
        const response = await request.post("news", form_data);
        this.get();
        if (response.status) {
          this.closeAddModal();
        }
      ...
      async edit() {
        ...
        const response = await request.put(`news`, this.data.id, form_data);
        this.get();
        if (response.status) {
          this.closeEditModal();
        }
      ...
      async delete_item(app) {
        const response = await request.delete(`news`, app.id);
        this.get();
        console.log(response);
      },
      async get() {
        const response = await request.post("news/paginate", {
          limit: this.limit,
          page: this.page,
        });
      ...
      async search() {
        const response = await request.post("news/search", {
          limit: 10,
          page: 1,
