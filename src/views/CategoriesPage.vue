      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <h3 class="font-bold text-gray-800 dark:text-white">اضف قسم</h3>
              <button
                type="button"
        ...
        >
          ...
          >
            <div
              class="flex justify-between items-center py-3 px-4 border-b dark:border-gray-700"
            >
              <h3 class="font-bold text-gray-800 dark:text-white">تعديل قسم</h3>
              <button
                type="button"
                class="flex justify-center items-center w-7 h-7 text-sm font-semibold rounded-full border border-transparent text-gray-800 hover:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                data-hs-overlay="#edit-modal"
...
export default {
  ...
  methods: {
    async add() {
      ...
      const response = await request.post("categories", form_data);
      this.get();
      console.log(response);
    },
    async edit() {
      const form_data = new FormData(this.$refs.edit_form);
      const response = await request.put(`categories`, this.data.id, form_data);
      this.get();
      console.log(response);
    },
    setModalData(app) {
    ...
    async delete_item(app) {
      const response = await request.delete(`categories/${app.id}`);
      this.get();
      console.log(response);
    },
    async get() {
      const response = await request.post("categories/paginate", {
        limit: this.limit,
        page: this.page,
      });
    ...
    async search() {
      const response = await request.post("categories/search", {
        limit: 10,
        page: 1,
